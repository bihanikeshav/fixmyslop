// Add cached Fontshare fonts to fonts.index.json as first-class entries
// (real metrics from local files, source=fontshare, fresh by default).
// Personality is filled afterwards by scripts/extend-personality-metrics.mjs.
...
const fams = JSON.parse(await readFile("data/external/fontshare/families.json", "utf8")) as Family[];
...
for (const fam of fams) {
  ...
  try {
    ...
    index.push({
      id: fam.id, family: fam.family, supplier: "fontshare", category: fam.category,
