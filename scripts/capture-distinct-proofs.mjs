...
for (const id of ids) {
  ...
  for (const viewport of [{ name: "desktop", width: 1440, height: 900 }, { name: "mobile", width: 390, height: 844 }]) {
    ...
    await page.goto("http://127.0.0.1:" + port + "/data/tmp/distinct-proofs/" + id + "/", { waitUntil: "networkidle" });
    ...
    const checks = await page.evaluate(() => ({
      ...
      overflowing: [...document.querySelectorAll("body *")].map((el) => {
        ...
        return { tag: el.tagName.toLowerCase(), text: String(el.textContent || "").trim().slice(0, 28), className: String(el.className || ""), left: Math.round(r.left), right: Math.round(r.right), width: Math.round(r.width) };
...
await browser.close();
server.close();
writeFileSync(path.join(OUT, "capture-report.json"), JSON.stringify({ schemaVersion: "quality-proof-capture.v3", generatedAt: new Date().toISOString(), results }, null, 2) + "\n");
