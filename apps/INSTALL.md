...
{
  "mcpServers": {
    "fixmyslop": { "url": "https://fixmyslop.bihanikeshav.workers.dev/mcp" }
  }
}
```
...
One-line skill install (drops the self-contained `fixmyslop` skill into your project):
```bash
...
The companion **`fixmyslop`** skill wires exactly this flow into a design process:
