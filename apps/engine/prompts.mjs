...
export const INSTRUCTIONS = "fix-ai-slop is a design engine: it judges and generates non-slop design tokens (color, fonts, spacing, radius, shadow, layout, motion) with deterministic math. When building or reviewing any UI, route every deterministic decision to these tools: understand the subject first, ask only what's genuinely unclear, call `design_system` for a coherent baseline or the per-domain tools, then audit with the `check_*` tools / `audit_system` and fix anything marked SLOP. The two rules that break most AI designs: FONTS — `suggest_fonts` returns a pairing; `pairing.body` is the readable face, NEVER set a display/novelty font as body text. LAYOUT — use the container tokens `layout` returns (maxWidth + paddingInline); NEVER re-add margin on top of `inner`. Connected flow: form a StyleIntent from the brief, normalize it with `resolve_intent`, then call `style_genome` for ONE coherent direction (fonts + palette + layout family + material together) — or several, feeding each shown direction's fingerprint back so they diverge. Guided workflow: 7 prompts via prompts/list — explore (show divergent directions to pick from), improve_design, design_review, theme, colorize, typeset, polish.";

export const VERBS = [
  {
    name: "explore",
    description: "Show the user several genuinely different design directions to pick from, grounded in their intent.",
    args: [{ name: "intent", description: "What you're building, who it's for, the feel (optional).", required: false }, { name: "centrepiece", description: "One line describing the standout you picture (optional).", required: false }],
